...
void PhoneBook::displayTable()
{
    ...
    std::cout << "----------|----------|----------|----------|" << std::endl;
    for (int i = 0; i < contactsLength && i < 8; i++)
        contacts[i].printAsRow(i);
}

bool PhoneBook::valid_phone_number(std::string str)
{
    for (int i = 0; str[i]; i++)
    {
...
void PhoneBook::addContract()
{
    ...
    contacts[contactsLength % 8].setContact(firstName, lastName, nickname, phoneNumber, secret);
    contactsLength++;
}
